...
class HostDiagnosticsService:
    ...
    def _inspect_git(self) -> HostToolDiagnostics:
        ...
        if not path:
            return self._missing_tool(
                ...
                steps=[
                    ...
                    "Ensure the host executor process can resolve `git` in PATH.",
                ],
            )
        ...
        if error_message:
            return self._error_tool(
                ...
                steps=[
                    "Run `git --version` in the same environment as the host executor.",
        ...
        if not version_ok:
            return self._outdated_tool(
                ...
                steps=[
                    ...
                    "Restart the host executor after upgrading.",
                ],
            )
    ...
    def _inspect_gh(self) -> HostToolDiagnostics:
        ...
        if error_message:
            return self._error_tool(
                ...
                message=error_message,
                steps=[
                    "Run `gh --version` directly in the same environment as the host executor.",
            ...
            return self._error_tool(
                ...
                steps=[
                    (
                        ...
                        "environment as the host executor."
                    ),
        ...
        if not auth_ok:
            return HostToolDiagnostics(
                ...
                remediation_steps=[
                    (
                        "Run `codex login` using browser or device auth as "
                        "the same OS user that starts the host executor."
                    ),
    ...
    def _inspect_tmux(self) -> HostToolDiagnostics:
        ...
        if not path:
            return self._missing_tool(
                ...
                steps=[
                    ...
                    "Ensure the host executor process can resolve `tmux` in PATH.",
                ],
            )
        ...
        if error_message:
            return self._error_tool(
                ...
                message=error_message,
                steps=[
                    "Run `tmux -V` in the same environment as the host executor.",
    ...
    def _build_executor_context(self) -> HostExecutorContext:
        """Return runtime information for the current process."""
    ...
    def _detect_container_runtime() -> str | None:
        """Infer whether the process is running in a known container runtime."""
    ...
    def _build_warnings(
        ...
        *,
        ...
        if executor_context.containerized:
            ...
            warnings.append(
                f"The host executor process is running inside {runtime}. "
                "This defeats the intended host-native execution model."
            )

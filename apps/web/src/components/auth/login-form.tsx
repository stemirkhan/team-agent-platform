...
export function LoginForm({ locale, allowOpenRegistration }: LoginFormProps) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await loginWithPassword(
        ...
        {
          fallbackMessage: t(locale, { ru: "Не удалось выполнить вход.", en: "Login failed." })
        }
      );
      ...
      setErrorMessage(
        error instanceof Error ? error.message : t(locale, { ru: "Не удалось выполнить вход.", en: "Login failed." })
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <section className="mx-auto max-w-md rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900/95 p-6 shadow-lg shadow-slate-200/60">
      ...
      <h1 className="mb-2 text-2xl font-black text-slate-900 dark:text-slate-50">{t(locale, { ru: "Вход", en: "Login" })}</h1>
      <p className="mb-6 text-sm text-slate-600 dark:text-slate-300">
        {t(locale, { ru: "Используйте данные своей учетной записи, чтобы продолжить.", en: "Use your account credentials to continue." })}
      </p>

      <form className="space-y-4" onSubmit={onSubmit}>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
          {t(locale, { ru: "Email", en: "Email" })}
        ...
        <Button disabled={submitting} type="submit">
          {submitting ? t(locale, { ru: "Выполняется вход...", en: "Signing in..." }) : t(locale, { ru: "Войти", en: "Sign in" })}
        </Button>
      </form>

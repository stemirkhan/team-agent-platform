"""GitHub tracker endpoints proxied through the host executor."""
...
def list_repos(
    ...
    """Return repositories visible through the host executor."""
    try:
        return github_proxy_service.list_repos(owner=owner, limit=limit, query=search)
...
def get_repo(
    ...
    try:
        return github_proxy_service.get_repo(owner=owner, repo=repo)
    except GitHubProxyServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
...
def get_issue(
    ...
    try:
        return github_proxy_service.get_issue(owner=owner, repo=repo, number=number)
    except GitHubProxyServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
...
def add_issue_comment(
    ...
    try:
        return github_proxy_service.add_comment(
            owner=owner,
            repo=repo,
            number=number,
            payload=payload,
...
def add_issue_labels(
    ...
    try:
        return github_proxy_service.add_labels(
            owner=owner,
            repo=repo,
            number=number,
            payload=payload,
...
def remove_issue_label(
    ...
    try:
        return github_proxy_service.remove_label(
            owner=owner,
            repo=repo,
            number=number,
...
def list_pulls(
    ...
    try:
        return github_proxy_service.list_pulls(owner=owner, repo=repo, state=state, limit=limit)
    except GitHubProxyServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
...
def get_pull(
    ...
    try:
        return github_proxy_service.get_pull(owner=owner, repo=repo, number=number)
    except GitHubProxyServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
...
def get_pull_checks(
    ...
    """Return normalized checks for one repository pull request."""
    try:
        return github_proxy_service.get_pull_checks(owner=owner, repo=repo, number=number)
    except GitHubProxyServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc

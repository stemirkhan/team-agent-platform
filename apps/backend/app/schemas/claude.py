"""Schemas for backend Claude terminal proxy endpoints."""
...
class ClaudeSessionStart(BaseModel):
    """Payload for starting one Claude session through the host executor."""
...
class ClaudeSessionRead(BaseModel):
    """Current host-side Claude session state."""
...
class ClaudeTerminalChunk(BaseModel):
    """One raw terminal output chunk from Claude Code."""
